...
function Facture({ data, shop }: Props) {
  const generatePDF = () => {
    ...
    doc.text(
      `FACTURE PROFORMA N° ${data[0].idFacture}`,
      pageWidth - margin - 60,
      yPosition
    );
    yPosition += 5;
    ...
    const tableData = data.map((item, index) => [
      ...
      item.produit,
      item.quantite,
      `${item.prix_achat.toLocaleString()} CDF`,
      `${(item.prix_vente * item.quantite).toLocaleString()} CDF`,
    ]);
  ...
  return (
    <article className="page bg-white rounded-2xl shadow-2xl no-print-shadow p-8 sm:p-12 mb-10 print:rounded-none print:shadow-none max-w-4xl mx-auto print:h-screen print:overflow-hidden ">
      ...
      <section className="border-b-4 border-[#00A651] pb-6 mb-6 print:pb-4 print:mb-4 print:flex-shrink-0">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right ml-6 print:ml-4">
            ...
            <div className="bg-[#703307] text-white p-3 rounded-lg print:p-2">
                ...
                data.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="border border-gray-200 px-2 py-2 text-right text-gray-700 font-semibold print:px-1 print:py-1">
                      {item.prix_vente * item.quantite} CDF
                    </td>
                  </tr>
                ))}
            </tbody>
      ...
      {/* PDF Download Button */}
      <section className="flex gap-3 mt-6 print:hidden">
        <button
          onClick={generatePDF}
        ...
        {!data[0].considerer && (
          <Popup
            title={`Le client doit payer ${data[0].prix_vente} CDF`}
            description=""
            component={
              <Payer
                data={data}
                montant={data[0].prix_vente}
                client={data[0].codeclient}
              />
            }
            btnname="Valider la facture"
          />
        )}
      </section>
